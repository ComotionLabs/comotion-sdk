...
This helper calls the `/dailyRun/execution_status` endpoint on the Dash API and returns an `ExecutionStatus` model.
...
`ExecutionStatus` – a generated model in which every field is optional, because which fields are populated depends on the `mode` used.
...
`LATEST` returns `None` when the organisation has no executions at all, because the API responds with a null body in that case.

Model fields are snake_case where the JSON uses camelCase. Use `.to_dict()` for the original shape:
